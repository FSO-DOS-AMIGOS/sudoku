//compile usando: $ gcc sudoku_debug.c -o sudoku_debug -lpthread 
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[])
{
   ...
   //Criando as threads para executar a validação por setor(linha,coluna,grid)
    ...
    if(n_iguais == 11){
    	...
    	printf("\nThread-Pai: Não valido o sudoku\n");
...
//debugando o vetor
 printf("vector_validacao: "); 
for(i=0;i<9;i++)
	printf("%i",vector_validacao[i]);
	printf("\n");
...
for(i=1;i<=3;i++){
    for(j=1;j<=3;j++){
        ...
        //verificando se está correto (debug)
